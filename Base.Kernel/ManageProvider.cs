...
namespace Base.Kernel
{
    ...
    public class ManageProvider : IManageProvider
    {
        ...
        /// </summary>
        private string LoginProvider = ConfigHelper.LoginProvider;
        /// <summary>
        /// 写入登录信息
        /// </summary>
        ...
        public virtual void AddCurrent(IManageUser user)
        {
            try
            {
                ...
                {
                    CookieHelper.SetCookie(LoginUserKey, DESEncryptHelper.Encrypt(JsonHelper.ToJson(user)), 1440);
                }
                else
                {
                    SessionHelper.Add(LoginUserKey, DESEncryptHelper.Encrypt(JsonHelper.ToJson(user)));
                }
            }
            catch (Exception ex)
            {
